...
const MagneticCTAButton: React.FC<MagneticCTAButtonProps> = ({ children, onClick, type = 'button', disabled }) => {
  ...
  return (
    <motion.button
      ...
      className={`group relative inline-flex items-center gap-3 md:gap-5 pl-5 md:pl-9 pr-1.5 md:pr-2 py-1.5 md:py-2 rounded-full text-white font-semibold md:font-bold text-[11px] md:text-sm tracking-wider uppercase shadow-lg shadow-[#0066FF]/25 z-10 shrink-0 ${disabled ? 'opacity-70 cursor-not-allowed' : ''}`}
    ...
    >
      ...
      >
        ...
        >
          {disabled ? <CheckCircle2 className="w-3.5 h-3.5 md:w-5 md:h-5 text-white" /> : <ArrowUpRight className="w-3.5 h-3.5 md:w-5 md:h-5 text-white" />}
        </motion.span>
      </motion.span>
    </motion.button>
  );
      ...
      <div className="relative z-10">
        ...
        <section className="relative pt-[85px] pb-6 md:pb-20 px-4 sm:px-8 max-w-7xl mx-auto text-center overflow-hidden">
          ...
          <motion.p 
            ...
            className="relative text-sm sm:text-lg text-white/90 sm:text-white/80 max-w-2xl mx-auto mt-4 leading-relaxed"
          >
            <span className="sm:hidden">
              Ready to eliminate hiring delays and payroll administrative overhead? Connect with us today.
            </span>
        ...
        {/* SPLIT-SCREEN LAYOUT */}
        <section className="pt-6 sm:pt-8 pb-32 px-4 sm:px-8 max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
            
            {/* LEFT SIDE: Contact Details & Quick Actions (Shifted to bottom on mobile via order-2) */}
            <div className="lg:col-span-5 space-y-8 flex flex-col order-2 lg:order-1">
              <TiltBlueCard>
                {/* Reduced padding and internal spacing purely for mobile */}
                <div className="p-6 sm:p-10 space-y-5 sm:space-y-8 h-full flex flex-col">
                  <h2 className="text-[26px] sm:text-3xl font-extrabold text-white tracking-tight drop-shadow-sm">
                  ...
                  <div className="space-y-4 sm:space-y-6 flex-1">
                    ...
                    <div className="flex items-start gap-3 sm:gap-4">
                      <div className="p-2.5 sm:p-3 rounded-2xl bg-white/10 border border-white/20 text-[#FFD84D] shrink-0 shadow-sm backdrop-blur-md">
                        <Phone className="w-5 h-5 sm:w-6 sm:h-6" />
                      </div>
                      <div>
                        <h4 className="font-bold text-[10px] sm:text-xs uppercase tracking-wider text-white/60">Phone / WhatsApp</h4>
                        <a href={`tel:${COMPANY_DETAILS.phone}`} className="text-base sm:text-lg font-bold text-white hover:text-[#FFD84D] transition-colors block mt-0.5 sm:mt-1">
                        ...
                        <a 
                          ...
                          className="inline-flex items-center gap-1.5 text-[11px] sm:text-xs text-[#FFD84D] font-semibold mt-1 hover:underline drop-shadow-sm"
                        >
                    ...
                    <div className="flex items-start gap-3 sm:gap-4">
                      ...
                      <div>
                        <h4 className="font-bold text-[10px] sm:text-xs uppercase tracking-wider text-white/60">Registered Address</h4>
                        <a 
                          href="https://www.google.com/maps/search/?api=1&query=47/65+Mettu+Street,+Vadiveeswaram,+Nagercoil-629002"
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-xs sm:text-sm font-medium text-white/90 leading-relaxed mt-0.5 sm:mt-1 hover:text-[#FFD84D] transition-colors block"
                        >
                    ...
                    <div className="flex items-start gap-3 sm:gap-4">
                      ...
                      <div>
                        <h4 className="font-bold text-[10px] sm:text-xs uppercase tracking-wider text-white/60">Email Enquiries</h4>
                        <a href={`mailto:${COMPANY_DETAILS.email}`} className="text-[13px] sm:text-sm font-semibold text-white hover:text-[#FFD84D] transition-colors block mt-0.5 sm:mt-1">
                  ...
                  <div className="pt-4 sm:pt-6 border-t border-white/20">
                    <h4 className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-white/60 mb-3 sm:mb-4">
                      Follow Social Media
                    </h4>
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                      ...
                      <a 
                        ...
                        className="px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl sm:rounded-2xl bg-white/10 border border-white/20 hover:bg-white/20 text-white transition-all shadow-sm flex items-center gap-1.5 sm:gap-2 text-[11px] sm:text-xs font-semibold backdrop-blur-md"
                      >
                        <Linkedin className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-blue-300" />
                        <span>LinkedIn</span>
                      </a>
              ...
              {/* Desktop Guaranteed Response Time Wrapper - Hidden on Mobile */}
              <div 
                className="hidden lg:block p-6 sm:p-8 rounded-[2.25rem] border border-white/20 text-white backdrop-blur-md shadow-xl"
                style={{ 
                  background: 'linear-gradient(135deg, rgba(15,103,255,0.75) 0%, rgba(6,48,145,0.85) 100%)',
            ...
            {/* RIGHT SIDE: Highly Transparent Whitish Glassy Form (Shifted to top on mobile via order-1) */}
            <div className="lg:col-span-7 h-full order-1 lg:order-2">
                  ...
                  {!isSuccess ? (
                    <form onSubmit={handleSubmit} className="space-y-6">
                      <div>
                        ...
                        <h3 className="text-[22px] sm:text-4xl font-extrabold text-[#1D1D1F] tracking-tight mt-1 sm:mt-2">
              ...
              {/* MOBILE ONLY: Guaranteed Response Time (Moved to Bottom of the right column, above HQ) */}
              <div 
                className="block lg:hidden mt-6 p-5 rounded-2xl border border-white/20 text-white backdrop-blur-md shadow-xl"
                style={{ 
                  background: 'linear-gradient(135deg, rgba(15,103,255,0.75) 0%, rgba(6,48,145,0.85) 100%)',
                  boxShadow: 'inset 0 1px 2px rgba(255,255,255,0.25), 0 10px 30px -10px rgba(15,103,255,0.3)'
                }}
              >
                <div className="flex items-center gap-2 font-extrabold text-sm tracking-wide mb-2 drop-shadow-sm">
                  <Clock className="w-4 h-4 text-[#FFD84D]" />
                  Guaranteed Response Time
                </div>
                <p className="text-white/90 leading-relaxed text-xs font-medium">
                  All business proposals submitted through this lead portal receive a dedicated call and customized proposal from our senior talent lead within <strong className="text-white font-extrabold">2 hours</strong>.
                </p>
              </div>
