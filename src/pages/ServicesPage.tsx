...
// ----------------------------------------------------------------------
// Premium Magnetic CTA Button (Responsive: Matches mobile Contact button, keeps desktop size)
// ----------------------------------------------------------------------
...
const MagneticCTAButton: React.FC<MagneticCTAButtonProps> = ({ children, onClick }) => {
  ...
  return (
    <motion.button
      ...
      className="group relative inline-flex items-center gap-3 md:gap-5 pl-5 md:pl-9 pr-1.5 md:pr-2 py-1.5 md:py-2 rounded-full text-white font-semibold md:font-bold text-[11px] md:text-sm tracking-wider uppercase shadow-lg shadow-[#0066FF]/25 z-10 shrink-0"
...
const ServiceModuleCard: React.FC<{
  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 p-6 sm:p-12 flex flex-col gap-4 sm:gap-6">
        <div className="flex flex-col md:flex-row items-start justify-between gap-4 sm:gap-8 pb-4 sm:pb-8 border-b border-white/20">
          <div className="flex items-start gap-3 sm:gap-5">
            <div className="p-2.5 sm:p-4 rounded-2xl bg-white shadow-lg shadow-black/20 shrink-0">
              <div className="text-[#0066FF]">{icon}</div>
            </div>
            <div>
              <span className="text-[10px] sm:text-xs font-bold uppercase tracking-widest text-[#FFD84D]">
                Service Module 0{idx + 1}
              </span>
              <h2 className="text-xl sm:text-4xl font-extrabold text-white tracking-tight mt-1 sm:mt-2 drop-shadow-sm">
                {srv.title}
              </h2>
              <p className="text-[11px] sm:text-sm text-white/80 font-semibold mt-1 sm:mt-2">
        ...
        <div className="py-1 sm:py-2 text-xs sm:text-base text-white/90 leading-relaxed font-light">
          {srv.fullDesc}
        </div>

        {/* Hidden on mobile view, shown on desktop view */}
        <div className="hidden md:grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8 pt-1 sm:pt-2">
            ...
            <ul className="space-y-3">
              {srv.features.map((ft: string, fIdx: number) => (
                <li key={fIdx} className="text-sm text-white/90 flex items-start gap-3 font-medium">
                  <span className="w-2 h-2 rounded-full bg-[#FFD84D] shrink-0 mt-2" />
                  <span>{ft}</span>
                </li>
              ))}
            </ul>
...
export const ServicesPage: React.FC<ServicesPageProps> = ({ onNavigate }) => {
  ...
  const serviceIconsMap: Record<string, React.ReactNode> = {
    'Users': <Users className="w-6 h-6 sm:w-8 sm:h-8 text-[#0066FF]" />,
    'Crown': <Crown className="w-6 h-6 sm:w-8 sm:h-8 text-[#0066FF]" />,
    'Receipt': <Receipt className="w-6 h-6 sm:w-8 sm:h-8 text-[#0066FF]" />,
    'ShieldCheck': <ShieldCheck className="w-6 h-6 sm:w-8 sm:h-8 text-[#0066FF]" />,
    'GraduationCap': <GraduationCap className="w-6 h-6 sm:w-8 sm:h-8 text-[#0066FF]" />
  };
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <section className="relative pt-[85px] pb-6 md:pb-20 px-4 sm:px-8 max-w-7xl mx-auto text-center overflow-hidden">
          ...
          >
            <span className="sm:hidden">
              Whether you need a single executive search or a complete outsourced HR squad, we deliver precision and speed.
            </span>
        ...
        {/* SERVICES SECTION - Removed scroll/sticky stacking effect on mobile, replaced with standard clean flex layout */}
        <section className="py-8 md:py-12 pb-24 px-4 sm:px-8 max-w-5xl mx-auto flex flex-col gap-6 md:gap-[25vh]">
          {SERVICES_DATA.map((srv, idx) => (
            <div
              key={srv.id}
              className="relative md:sticky md:mb-[25vh]"
              style={{
