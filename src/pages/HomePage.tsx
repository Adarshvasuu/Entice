...
const ServicesCTAButton: React.FC<ServicesCTAButtonProps> = ({ children, onClick }) => {
  const ref = useRef<HTMLButtonElement>(null);
  const [offset, setOffset] = useState({ x: 0, y: 0 });
  const [hovered, setHovered] = useState(false);
  ...
  return (
    <motion.button
      ...
      className="group relative inline-flex items-center gap-4 pl-7 pr-1.5 py-1.5 rounded-full text-white font-semibold text-xs tracking-wider uppercase shadow-lg shadow-[#0066FF]/25"
...
// ----------------------------------------------------------------------
// Cinematic "Why Entice" Feature Card
// ----------------------------------------------------------------------
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <section ref={heroSectionRef} className="relative min-h-screen flex flex-col justify-center px-4 sm:px-8 pt-[85px] pb-10 overflow-hidden">
          ...
          >
            <h1 className="text-5xl sm:text-7xl md:text-[5.5rem] leading-[1.1] font-extrabold tracking-tight text-center max-w-5xl mx-auto select-none text-white">
              {headlineText.split('').map((char, i) => {
                ...
                return (
                  <React.Fragment key={i}>
                    {/* ONLY SHOWS ON MOBILE TO FORCE LINE BREAK */}
                    {i === accentStartIndex && <br className="block sm:hidden" />}
          ...
          <div className="relative max-w-4xl mx-auto space-y-6 mt-6 text-center">
            {/* HIDDEN ON MOBILE, VISIBLE ON DESKTOP */}
            <motion.p 
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.7, delay: 0.2 }}
              className="hidden sm:block text-lg sm:text-xl text-white/90 max-w-2xl mx-auto font-light leading-relaxed pt-2"
            >
            ...
            <motion.div 
              ...
              className="pt-8 md:pt-10 flex flex-col sm:flex-row items-center justify-center gap-4"
            >
              <MagneticButton
                onClick={() => onNavigate('contact')}
                glowColor="rgba(255,255,255,0.5)"
                className="bg-white text-[#0F67FF] px-8 py-4 rounded-full text-xs font-semibold tracking-wide uppercase transition-colors flex items-center justify-center gap-2 group shadow-lg"
              >
                Partner With Us
              ...
              <MagneticButton
                ...
                className="px-8 py-4 rounded-full bg-white/10 hover:bg-white/20 text-white font-semibold text-xs tracking-wide uppercase transition-colors flex items-center justify-center gap-2 border border-white/30 group backdrop-blur-md"
              >
                Explore Services
                <ChevronRight className="w-4 h-4 text-white/80 group-hover:translate-x-1 group-hover:text-[#FFD84D] transition-all" />
              </MagneticButton>
            </motion.div>
          </div>
          ...
          <motion.div 
            ...
            className="relative flex flex-col w-full max-w-4xl mx-auto py-12 mt-12 border-y border-white/20 gap-10"
          >
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full gap-10 sm:gap-24 px-8">
              <motion.div style={{ x: leftShift }} className="text-left flex-1 border-l-2 border-white pl-6 w-full">
                <div className="text-4xl font-bold text-white drop-shadow-md">20M+</div>
                <div className="text-[14px] uppercase tracking-[0.2em] text-[#FFD84D]/90 font-medium mt-2">Candidate Database</div>
              </motion.div>
              <motion.div style={{ x: rightShift }} className="text-left flex-1 border-l-2 border-white pl-6 w-full">
                <div className="text-4xl font-bold text-white drop-shadow-md">15,000+</div>
                <div className="text-[14px] uppercase tracking-[0.2em] text-[#FFD84D]/90 font-medium mt-2">Successful Placements </div>
              </motion.div>
            </div>
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full gap-10 sm:gap-24 px-8">
              <motion.div style={{ x: leftShift }} className="text-left flex-1 border-l-2 border-white pl-6 w-full">
                <div className="text-4xl font-bold text-white drop-shadow-md">97%</div>
                <div className="text-[14px] uppercase tracking-[0.2em] text-[#FFD84D]/90 font-medium mt-2">Completion Rate </div>
              </motion.div>
              <motion.div style={{ x: rightShift }} className="text-left flex-1 border-l-2 border-white pl-6 w-full">
                <div className="text-4xl font-bold text-white drop-shadow-md">100+</div>
                <div className="text-[14px] uppercase tracking-[0.2em] text-[#FFD84D]/90 font-medium mt-2">Active Clients</div>
              </motion.div>
            </div>
          </motion.div>
        </section>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      zIndex: idx + 1,
                      transform: `rotate(${rotations[idx % rotations.length]}deg) translateX(${xOffsets[idx % xOffsets.length]}px)`,
                    }}
                  >
                    <TiltServiceCard
                      title={service.title}
        ...
        {/* CINEMATIC "WHY ENTICE HR" SCROLL SEQUENCE */}
        <section ref={cinematicWhyRef} className="relative h-[150vh] z-20">
          <div className="sticky top-0 h-screen w-full flex flex-col items-center justify-center px-4 overflow-hidden">
            ...
            >
              <h2 className="text-[32px] sm:text-[3rem] md:text-[5.5vw] xl:text-[5.5rem] font-black text-[#1D1D1F] text-center px-4 tracking-tighter drop-shadow-sm leading-[1.05]">
                Why <br />
                <span 
        ...
        */}
        <section className="relative py-16 md:py-24 px-4 sm:px-8 max-w-7xl mx-auto bg-white/30 backdrop-blur-2xl rounded-[2.5rem] border border-white/50 mb-12 z-10 shadow-[0_20px_60px_-15px_rgba(0,10,40,0.05)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {TESTIMONIALS.map((t) => (
              <div key={t.id} className="p-8 rounded-[2rem] bg-white/60 backdrop-blur-md border border-white/80 shadow-xl flex flex-col justify-between hover:scale-[1.02] transition-transform duration-300">
                <div>
