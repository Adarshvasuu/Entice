...
// ----------------------------------------------------------------------
// Reusable CTA Button (Copied from Home Page, Size Increased)
// ----------------------------------------------------------------------
...
const GrowWithUsButton: React.FC<GrowWithUsButtonProps> = ({ children, onClick }) => {
  ...
  return (
    <motion.button
      ...
      className="group relative inline-flex items-center gap-5 pl-9 pr-2 py-2 rounded-full text-white font-bold text-sm tracking-wider uppercase shadow-lg shadow-[#0066FF]/25"
  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 p-8 sm:p-10 space-y-4">
        <div className="w-12 h-12 rounded-2xl bg-[#0066FF]/10 text-[#0066FF] flex items-center justify-center backdrop-blur-sm">
          {icon}
        </div>
        <h3 className="text-2xl font-bold text-[#1D1D1F] tracking-tight">{title}</h3>
        <p className="text-sm text-[#3A3A3E] leading-relaxed">{desc}</p>
      </div>
    </motion.div>
  );
};
...
export const AboutPage: React.FC<AboutPageProps> = ({ onOpenConsultation, onNavigate }) => {
  ...
  const journeySteps = [
    ...
    },
    {
      step: '2024',
      title: 'AI Expansion',
      desc: 'Launched dedicated AI consulting and tech infrastructure practices to serve the next generation of businesses. Added ML strategy and automation to our core offering.',
      icon: <Cpu className="w-5 h-5 text-[#0066FF]" />
    },
    {
      step: '2025',
      title: '50+ Clients',
      desc: 'Delivered 150K+ leads and 100K+ impressions. Became a trusted growth partner for premium brands across India with a full suite of 7 core services.',
    ...
    >
      ...
      <section className="relative pt-[85px] pb-16 md:pb-20 px-4 sm:px-8 max-w-7xl mx-auto text-center overflow-hidden">
        <motion.h1 
          ...
          className="relative mt-20 md:mt-32 text-4xl sm:text-6xl font-extrabold text-white tracking-tight leading-tight"
        >
      ...
      {/* MISSION & VISION */}
      <section className="py-16 px-4 sm:px-8 max-w-7xl mx-auto border-t border-black/[0.06]">
      ...
      <section className="relative py-16 md:py-24 px-4 sm:px-8 max-w-7xl mx-auto border-t border-black/[0.06]">
        ...
        {/* BUTTON FIX: Animated Magnetic Button copied from HomePage */}
        <div className="mt-20 md:mt-28 text-center flex justify-center pb-10">
